...
- (BOOL)_buildString
{
	...
	__block BOOL result;
	dispatch_group_async(_dataParsingGroup, _dataParsingQueue, ^{ result = [self->_parser parse]; });
...
- (void)_registerTagStartHandlers
{
	...
	void (^pBlock)(void) = ^
	{
		...
		{
			self->_currentTag.paragraphStyle.firstLineHeadIndent = self->_currentTag.paragraphStyle.headIndent + self->_defaultParagraphStyle.firstLineHeadIndent;
		}
		else
		{
...
- (void)parser:(DTHTMLParser *)parser didStartElement:(NSString *)elementName attributes:(NSDictionary *)attributeDict
{
	...
	dispatch_group_async(_treeBuildingGroup, _treeBuildingQueue, ^{
		...
		if (self->_ignoreParseEvents)
		{
			return;
		}

		DTHTMLElement *newNode = [DTHTMLElement elementWithName:elementName attributes:attributeDict options:self->_options];
		DTHTMLElement *previousLastChild = nil;
		
		if (self->_currentTag)
		{
			// inherit stuff
			[newNode inheritAttributesFromElement:self->_currentTag];
			[newNode interpretAttributes];
			
			previousLastChild = [self->_currentTag.childNodes lastObject];
			
			// add as new child of current node
			[self->_currentTag addChildNode:newNode];
			
			// remember body node
			if (!self->_bodyElement && [newNode.name isEqualToString:@"body"])
			{
				self->_bodyElement = newNode;
			}
			
			if (self->_shouldProcessCustomHTMLAttributes)
			{
				newNode.shouldProcessCustomHTMLAttributes = self->_shouldProcessCustomHTMLAttributes;
			}
		}
		else
		{
			NSAssert(!self->_rootNode, @"Something went wrong, second root node found in document and not ignored.");
			...
			if (!self->_rootNode)
			{
				...
				[self->_rootNode inheritAttributesFromElement:self->_defaultTag];
				[self->_rootNode interpretAttributes];
			}
		}
		
		// apply style from merged style sheet
		NSSet *matchedSelectors;
		NSDictionary *mergedStyles = [self->_globalStyleSheet mergedStyleDictionaryForElement:newNode matchedSelectors:&matchedSelectors ignoreInlineStyle:self->_ignoreInlineStyles];
		...
		if (previousLastChild && newNode.displayStyle != DTHTMLElementDisplayStyleInline)
		{
			if ([previousLastChild isKindOfClass:[DTTextHTMLElement class]])
			{
				...
				{
					[self->_currentTag removeChildNode:textElement];
				}
			}
		}
		
		self->_currentTag = newNode;
		
		// find block to execute for this tag if any
		void (^tagBlock)(void) = [self->_tagStartHandlers objectForKey:elementName];
...
- (void)parser:(DTHTMLParser *)parser didEndElement:(NSString *)elementName
{
	...
	dispatch_group_async(_treeBuildingGroup, _treeBuildingQueue, ^{
		@autoreleasepool {
			...
			// find block to execute for this tag if any
			void (^tagBlock)(void) = [self->_tagEndHandlers objectForKey:elementName];
			...
			if (self->_currentTag.displayStyle != DTHTMLElementDisplayStyleNone)
			{
				if (self->_currentTag == self->_bodyElement || self->_currentTag.parentElement == self->_bodyElement)
				{
					DTHTMLElement *theTag = self->_currentTag;
					
					dispatch_group_async(self->_stringAssemblyGroup, self->_stringAssemblyQueue, ^{
						...
						if ([theTag needsOutput])
						{
							...
							if (self->_willFlushCallback)
							{
								self->_willFlushCallback(theTag);
							}
							...
							if (nodeString)
							{
								...
								{
									if ([self->_tmpString length] && ![[self->_tmpString string] hasSuffix:@"\n"])
									{
										// trim off whitespace
										while ([[self->_tmpString string] hasSuffixCharacterFromSet:[NSCharacterSet ignorableWhitespaceCharacterSet]])
										{
											[self->_tmpString deleteCharactersInRange:NSMakeRange([self->_tmpString length]-1, 1)];
										}
										
										[self->_tmpString appendString:@"\n"];
									}
								}
								
								[self->_tmpString appendAttributedString:nodeString];
								theTag.didOutput = YES;
								
								if (!self->_shouldKeepDocumentNodeTree)
								{
			...
			while (![self->_currentTag.name isEqualToString:elementName])
			{
				// missing end of element, attempt to recover
				self->_currentTag = [self->_currentTag parentElement];
			}
			
			// closing the root node, ignore everything afterwards
			if (self->_currentTag == self->_rootNode)
			{
				self->_ignoreParseEvents = YES;
			}
...
- (void)parser:(DTHTMLParser *)parser foundCharacters:(NSString *)string
{
	dispatch_group_async(_treeBuildingGroup, _treeBuildingQueue, ^{
		...
		NSAssert(self->_currentTag, @"Cannot add text node without a current node");
		
		if (!self->_currentTag.preserveNewlines && [string isIgnorableWhitespace])
		{
			// ignore whitespace as first element of block element
			if (self->_currentTag.displayStyle!=DTHTMLElementDisplayStyleInline && ![self->_currentTag.childNodes count])
			{
				return;
			}
			
			// ignore whitespace following a block element
			DTHTMLElement *previousTag = [self->_currentTag.childNodes lastObject];
		...
		[textNode inheritAttributesFromElement:self->_currentTag];
		[textNode interpretAttributes];
		
		// save it for later output
		[self->_currentTag addChildNode:textNode];
		
		DTHTMLElement *theTag = self->_currentTag;
		
		// text directly contained in body needs to be output right away
		if (theTag == self->_bodyElement)
		{
			dispatch_group_async(self->_stringAssemblyGroup, self->_stringAssemblyQueue, ^{
				[self->_tmpString appendAttributedString:[textNode attributedString]];
				theTag.didOutput = YES;
			});
...
{
	dispatch_group_async(_treeBuildingGroup, _treeBuildingQueue, ^{
		NSAssert(!self->_currentTag, @"Something went wrong, at end of document there is still an open node");

		if (!self->_preserverDocumentTrailingSpaces) {
			dispatch_group_async(self->_stringAssemblyGroup, self->_stringAssemblyQueue, ^{
				// trim off white space at end
				while ([[self->_tmpString string] hasSuffixCharacterFromSet:[NSCharacterSet whitespaceCharacterSet]])
				{
					[self->_tmpString deleteCharactersInRange:NSMakeRange([self->_tmpString length]-1, 1)];
				}
			});
