	...
	for (NSUInteger i=0; i<[paragraphs count]; i++)
	{
		...
		if (effectiveListStyle)
		{
			...
			[listsToOpen enumerateObjectsUsingBlock:^(DTCSSListStyle *oneList, NSUInteger idx, BOOL *stop) {
				...
				CGFloat listPadding = (paragraphStyle.headIndent - paragraphStyle.firstLineHeadIndent) / self->_textScale;
		...
		[HTMLAttributes enumerateKeysAndObjectsUsingBlock:^(NSString *key, id value, BOOL *stop) {
			...
			NSRange attributeEffectiveRange = [self->_attributedString rangeOfHTMLAttribute:key atIndex:paragraphRange.location];
		...
		[_attributedString enumerateAttributesInRange:paragraphRange options:0 usingBlock:^(NSDictionary *attributes, NSRange spanRange, BOOL *stopEnumerateAttributes) {
			...
			if ((spanURL || spanAnchorName) && (currentLinkRange.location == NSNotFound))
			{
				if (spanURL)
				{
					currentLinkRange = [self->_attributedString rangeOfLinkAtIndex:spanRange.location URL:NULL];
				}
				else if (spanAnchorName)
				{
					currentLinkRange = [self->_attributedString rangeOfAnchorNamed:spanAnchorName];
				}
				...
				NSDictionary *localHTMLAttributes = [self->_attributedString HTMLAttributesAtIndex:currentLinkRange.location];
				
				[localHTMLAttributes enumerateKeysAndObjectsUsingBlock:^(NSString *key, id value, BOOL *stopEnumerateKeysAndObjects) {
					
					// check if range is longer than current paragraph
					NSRange attributeEffectiveRange = [self->_attributedString rangeOfHTMLAttribute:key atIndex:currentLinkRange.location];
			...
			{
				NSRange prefixRange = [self->_attributedString rangeOfFieldAtIndex:spanRange.location];
			...
			if (!fontIsBlockLevel)
			{
				...
				if (fontDescriptor)
				{
					if (self->_textScale!=1.0f)
					{
						fontDescriptor.pointSize /= self->_textScale;
					}
			...
			CGFloat kerning = [attributes kerning] / self->_textScale;
			...
			[localHTMLAttributes enumerateKeysAndObjectsUsingBlock:^(NSString *key, id value, BOOL *stop) {
				...
				NSRange attributeEffectiveRange = [self->_attributedString rangeOfHTMLAttribute:key atIndex:spanRange.location];
